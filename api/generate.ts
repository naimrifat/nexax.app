...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        // Clean and parse the AI response from Make.com
        const rawData = await makeResponse.json();

        const cleanJsonBlock = (str: string) => {
            return JSON.parse(
                str.replace(/```json\n?/gi, '').replace(/```$/gi, '').trim()
            );
        };

        const titles = cleanJsonBlock(rawData.titles_string);
        const specifics = cleanJsonBlock(rawData.specifics_string);
        const description = rawData.description;

        // Return clean, structured result to the browser
