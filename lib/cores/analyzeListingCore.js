...
import { mapDetectedToAspects } from '../ebay/mapDetectedToAspects.js'
import { reconcileSpecificsCore } from './reconcileSpecificsCore.js'
...
Return JSON only:
{
  ...
  const rawSize = String(detected.size || '').trim()
  const sizeAlias = normalizeSizeAlias(rawSize)
  const size = sizeAlias.canonical || rawSize
  ...
  if (detected && Array.isArray(aspects)) {
    aiSpecifics = mapDetectedToAspects({ detected, aspects })
  }
  const mergedSpecificsResult = await reconcileSpecificsCore({
    payload: { item_specifics: finalSpecifics, aiSpecifics },
  })
  const mergedSpecifics = Array.isArray(mergedSpecificsResult?.data?.item_specifics)
    ? mergedSpecificsResult.data.item_specifics
    : finalSpecifics
  ...
  const payloadOut = {
    ...
    item_specifics: mergedSpecifics.map((s) => {
