...
export async function ebayCategoriesCore(input) {
  ...
  if (action === 'getCategoryConditions') {
    const requestId = String(Date.now())
    if (!cid) {
      return { ok: false, error: 'MISSING_CATEGORY_ID', status: 400, requestId }
    }
    try {
      const token = await getEbayAppToken()
      if (!token) {
        return { ok: false, error: 'EBAY_CONDITIONS_ERROR', status: 401, requestId }
      }
      ...
      if (!conditionsRes.ok) {
        return {
          ...
          details: String(conditionsText || '').slice(0, 200),
          requestId,
        }
      }
      ...
      return {
        ...
        error: 'EBAY_CONDITIONS_ERROR',
        status: 500,
        details: String(err?.message || 'Failed to fetch conditions').slice(0, 200),
        requestId,
      }
    }
  }
    ...
    try {
      ...
      console.log(
        JSON.stringify({
          ...
          categoryId: cid,
          ebayKeys: Object.keys(data || {}),
        })
      )

      const rawAspects = Array.isArray(data?.aspects)
        ? data.aspects
        : Array.isArray(data?.aspectGroups)
          ? data.aspectGroups.flatMap((g) => g?.aspects || g?.localizedAspects || [])
          : []

      const aspects = rawAspects
        .map((aspect) => {
          const name = String(aspect?.localizedAspectName || aspect?.aspectName || aspect?.name || '').trim()
          if (!name) return null
          const rawVals = aspect?.aspectValues || aspect?.values || []
          const values = Array.isArray(rawVals)
            ? rawVals.map((v) => String(v?.localizedValue || v?.value || v).trim()).filter(Boolean)
            : []
          const mode = String(aspect?.aspectConstraint?.aspectMode || '').toUpperCase()
          const multi = mode === 'MULTI' || aspect?.multi === true
          const required = !!(aspect?.aspectConstraint?.aspectRequired ?? aspect?.required)
