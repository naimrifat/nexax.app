// Production-grade analyzeListing core (TS wrapper for runtime stability)
export async function analyzeListingCore(input: any) {
  ...
  if (Array.isArray(rawSpecs)) {
    ...
    item_specifics = Object.entries(rawSpecs).map(([name, value]) => ({ name, value: (value as any) ?? '', multi: false }))
  }
  let keywords: any[] = []
