...
function normalizeSizeToAllowed(input, allowedValues) {
  ...
  const candidates = []
  const xCount = normalizedInput.match(/^X+L$/)
  if (xCount) {
    const count = normalizedInput.length - 1
    if (count >= 2) candidates.push(`${count}XL`)
  }

  const digitX = normalizedInput.match(/^([2-9])X$/)
  if (digitX) candidates.push(`${digitX[1]}XL`)
