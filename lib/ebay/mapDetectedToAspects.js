import { normalizeSizeAlias } from '../sizeAliasNormalize.js'
...
function normalizeSizeToAllowed(input, allowedValues) {
  const allowed = Array.isArray(allowedValues) ? allowedValues : []
  if (!input || !allowed.length) return { size: null, sizeType: null }

  const allowedMap = new Map(allowed.map((v) => [normalizeSizeStr(v), v]))
  const normalizedInput = normalizeSizeStr(input)
  const direct = allowedMap.get(normalizedInput)
  if (direct) return { size: direct, sizeType: null }

  const alias = normalizeSizeAlias(input)
  const candidates = [...alias.candidates]
  if (/XLT$/.test(normalizedInput)) {
    const xltNum = normalizedInput.match(/^([2-9])XLT$/)
    if (xltNum) candidates.push(`${xltNum[1]}XLT`)
    if (normalizedInput === 'XLT') candidates.push('XLT')
  }
  candidates.push(input)

  for (const candidate of candidates) {
    const matched = allowedMap.get(normalizeSizeStr(candidate))
    if (matched) return { size: matched, sizeType: null }
  }

  return { size: null, sizeType: null }
}

function inferSizeTypeFromEvidence(raw, allowed) {
  const evidence = normalizeTextForMatch(raw)
  if (!evidence) return ''

  const hasBig = /\bBIG\b/.test(evidence)
  const hasTall = /\bTALL\b/.test(evidence) || /\bXLT\b/.test(evidence) || /\b(?:ST|MT|LT)\b/.test(evidence)
  if (!hasBig && !hasTall) return ''

  const allowedMap = new Map((Array.isArray(allowed) ? allowed : []).map((v) => [normalizeValue(v), v]))
  if (hasBig && allowedMap.has('big & tall')) return allowedMap.get('big & tall')
  if (hasBig && allowedMap.has('big tall')) return allowedMap.get('big tall')
  if (hasTall && allowedMap.has('tall')) return allowedMap.get('tall')
  if (hasBig && allowedMap.has('big')) return allowedMap.get('big')
  return ''
}
...
function extractSizeTokensFromText(text, allowNumeric) {
  ...
  addMatches(/\bS\s*\/\s*P\b/g)
  addMatches(/\bS\s+P\b/g)
  addMatches(/\bSML\b/g)
  addMatches(/\bMD\b/g)
  addMatches(/\bMED\b/g)
  addMatches(/\bLG\b/g)
  addMatches(/\bL\s*\/\s*G\b/g)
  addMatches(/\bL\s+G\b/g)
  addMatches(/\bLG\s*\/\s*G\s*\/\s*G\b/g)
...
export function mapDetectedToAspects({ detected, aspects }) {
  ...
  const generatedTitle = detected?.generatedTitle || detected?.generated_title
  const listingTitle = detected?.listingTitle || detected?.listing_title

  console.log('[mapDetectedToAspects] CALLED', {
    detectedSize: detected?.size,
    title,
    aspectsCount: Array.isArray(aspects) ? aspects.length : null,
  })
  ...
  console.log(
    '[mapDetectedToAspects] HIT',
    {
      hasDetected: !!detected,
      detectedSize: detected?.size,
      title: title || generatedTitle || listingTitle || null,
    }
  )
  ...
  for (const aspect of list) {
    ...
    if (norm(name).includes('size type')) {
      if (selectionOnly && allowed.length) {
        const evidence = det.sizeTypeHint || det.size || title || generatedTitle || listingTitle || ''
        const inferred = inferSizeTypeFromEvidence(evidence, allowed)
    ...
    if (norm(name).includes('size') && selectionOnly && allowed.length) {
      const directResult = normalizeSizeToAllowed(det.size || '', allowed)
      if (directResult.size) {
        out.push({ name, value: directResult.size })
        continue
      }

      const canonMap = buildCanonMap(allowed)
      const allowNumeric = allowed.some((v) => /^\d+$/.test(normalizeSizeStr(v)))
      const titleText = title || generatedTitle || listingTitle || ''
      ...
      for (const src of sources) {
        ...
        for (const token of tokens) {
          const direct = normalizeSizeToAllowed(token, allowed)
          if (direct.size) {
            resolved = direct.size
            break
          }
  ...
  const aiSpecifics = out
  console.log('[mapDetectedToAspects] RETURNING', aiSpecifics)
  return aiSpecifics
}
